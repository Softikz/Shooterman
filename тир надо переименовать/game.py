...
# Состояния игры (NEW: добавлено DUEL)
MENU = 0
LEVEL1 = 1
LEVEL2 = 2
SHOP = 3
...
class Player(pg.sprite.Sprite):
    ...
    # NEW: метод для управления в дуэли
    def update_duel(self, keys, is_player1=True):
        if is_player1:
            if keys[K_w] and self.rect.top > 0:
                self.rect.y -= self.speed
            if keys[K_s] and self.rect.bottom < HEIGHT:
...
class MainMenu:
    def __init__(self):
        ...
        self.duel_button = Button(pg.Surface((300, 60)), (WIDTH // 2 - 150, HEIGHT // 2 + 200), "Дуэль")  # NEW
    ...
    def draw(self, surface):
        ...
        self.duel_button.draw(surface)  # NEW
    ...
    def handle_event(self, event):
        if event.type == MOUSEBUTTONDOWN and event.button == 1:
            if self.level1_button.is_clicked(event.pos):
                ...
                return SHOP
            elif self.duel_button.is_clicked(event.pos):  # NEW
                return DUEL
        return MENU
...
def game_duel():
    ...
    player1.bullets_in_magazine = 10

    player2 = Player()
    player2.rect.center = (600, HEIGHT // 2)
    player2.current_weapon = "pistol"
    player2.bullets_in_magazine = 10

    all_sprites.add(player1, player2, player1.weapon, player2.weapon)
    ...
    while running:
        for event in pg.event.get():
            if event.type == QUIT:
                ...
                if event.button == 1:  # ЛКМ - игрок 1
                    bullet = player1.shoot()
                    if bullet:
                        all_sprites.add(bullet)
                        bullets_p1.add(bullet)
                elif event.button == 3:  # ПКМ - игрок 2
                ...
                if event.key == K_ESCAPE:
                    ...
                    player1.reload()
                elif event.key == K_l:  # Перезарядка игрока 2
                    player2.reload()

        keys = pg.key.get_pressed()
        player1.update_duel(keys, is_player1=True)
        player2.update_duel(keys, is_player1=False)
        ...
        for bullet in bullets_p1:
            if pg.sprite.collide_rect(bullet, player2):
                player2.kill()
                running = False
        for bullet in bullets_p2:
            if pg.sprite.collide_rect(bullet, player1):
                player1.kill()
                running = False
        ...
        font = pg.font.SysFont(None, 36)
        p1_text = font.render("Игрок 1: WASD + ЛКМ", True, WHITE)
        p2_text = font.render("Игрок 2: Стрелки + ПКМ", True, WHITE)
